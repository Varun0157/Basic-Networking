...
void receiveData(int sock, struct sockaddr_in* client_addr, int* numChunks,
  ...
  *numChunks = atoi(buffer);
  ...
  if (sendto(sock, buffer, CHUNK_SIZE, 0, (struct sockaddr*)client_addr, addr_size) < 0) {
    perror("sendto");
    close(sock);
    exit(1);
  }
  ...
  *receivedData = (char**)malloc(sizeof(char*) * (*numChunks));
  ...
  // comment out this first send to ensure retransmitted kill ack is received and working
  ACK ack;
  ack.seqNum = -1;
  if (sendto(sock, &ack, sizeof(ack), 0, (struct sockaddr*)client_addr, addr_size) < 0) {
    perror("sendto");
    close(sock);
    exit(1);
  }
  ...
  while (true) {
    ...
    if (activity < 0) {
      ...
      if (packet.kill) {
        printf(TCBBLU "Kill received again, sending acknowledgement and breaking\n" RESET);
        ACK ack;
        ack.seqNum = -1;
        if (sendto(sock, &ack, sizeof(ack), 0, (struct sockaddr*)client_addr, addr_size) <
            0) {
  ...
  printf(TCBCYN "Receiver: Ending after %ld usec as we did not receive a packet\n" RESET, 2*getWaitTime() + 1000);
}
